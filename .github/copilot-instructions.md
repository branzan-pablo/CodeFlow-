# Angular 20 Blog - AI Agent Guide

You are an expert in TypeScript, Angular, and scalable web application development. Write sustainable, performant, and accessible code following Angular and TypeScript best practices.

## Project Architecture

**Current Stack:**
- Angular 20 with **zoneless change detection** (`provideZonelessChangeDetection()`)
- **SSR + Prerendering** hybrid setup using Angular SSR
- **Standalone components only** (no NgModules)
- **Tailwind CSS v4** with new `@use "tailwindcss"` syntax
- **Express.js server** for SSR with Angular Node App Engine
- **Signals-first state management**

**Key Project Files:**
- `src/app/app.config.ts` - Client providers (zoneless detection, hydration)
- `src/app/app.config.server.ts` - Server config merging
- `src/app/app.routes.server.ts` - Defines `RenderMode.Prerender` for all routes
- `src/server.ts` - Express server with Angular SSR integration
- `src/styles.scss` - Tailwind v4 entry point (`@use "tailwindcss"`)

**Development Commands:**
```bash
npm start                 # Development server
npm run build            # Production build (SSR + static)
npm run serve:ssr:blog   # Run built SSR server
npm test                 # Unit tests with Karma/Jasmine
```

**Critical Architecture Decisions:**
- **All routes prerender by default** - Change `app.routes.server.ts` for dynamic routes
- **No Zone.js** - Signals trigger change detection, not zone patches
- **Component-first** - Each component owns its state via signals
